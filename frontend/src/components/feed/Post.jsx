...
const Post = ({
   ...
   const likeAndCommentOnPost = async () => {
      try {
         const res = await api.put(`add-likes-on-post/${id}`, {
            userId: user._id,
         });
         onLike();
      } catch (error) {
   ...
   return (
      <motion.div
         ...
         <div className="overflow-hidden transition-all duration-500 border glass-elevated rounded-3xl hover:shadow-3xl hover:shadow-blue-500/10 border-white/30 hover:border-white/50">
            <div className="relative p-8 ">
               {/* Gradient Overlay */}
               <div className="absolute inset-0 transition-opacity duration-500 opacity-0 rounded-3xl bg-gradient-to-br from-white/5 to-blue-50/10 group-hover:opacity-100" />
               ...
               <div className="relative z-10 flex items-start justify-between mb-6">
                  <div className="flex space-x-4">
                     <motion.div 
                        ...
                        <Avatar className="transition-all duration-300 shadow-lg w-14 h-14 ring-3 ring-white/50 hover:ring-blue-400/50">
                           <AvatarImage
                              src={
                                 authorOfPost?.profilePic
                              }
                              alt={authorOfPost?.name || "user"}
                           />
                  ...
                  <motion.div
                     ...
                     <Button
                        variant="ghost"
                        size="sm"
                        className="p-3 rounded-full hover:bg-white/60 backdrop-blur-sm">
               ...
               <div className="relative z-10 space-y-6">
                  ...
                  {image && (
                     <motion.div
                        className="relative overflow-hidden shadow-lg rounded-3xl bg-gradient-to-br from-blue-50 to-purple-50 group"
                        whileHover={{ scale: 1.01, y: -2 }}
                        transition={{
                           duration: 0.3,
                           ease: [0.25, 0.25, 0, 1],
                        }}>
                        <div className="absolute inset-0 z-10 transition-opacity duration-300 opacity-0 rounded-3xl bg-gradient-to-t from-black/20 to-transparent group-hover:opacity-100" />
                        {/* Replace with your ImageWithFallback if needed */}
                        <img
                           src={image}
                           alt="Post image"
                           className="object-contain w-full transition-transform duration-500 rounded-3xl h-72 group-hover:scale-105"
                        />
                     </motion.div>
                  )}
               </div>
               ...
               {(likes.length > 0 || comments.length > 0) && (
                  <div className="relative z-10 flex items-center justify-between py-4 mt-6 border-t border-gray-100">
                     <div className="flex items-center space-x-4 text-sm">
                        {likes.length > 0 && (
                           <motion.div
                              className="flex items-center space-x-2 transition-colors cursor-pointer hover:text-blue-600 group"
                              whileHover={{ scale: 1.05 }}>
                              <div className="flex -space-x-1">
                                 <motion.div
                                    className="flex items-center justify-center w-6 h-6 border-2 border-white rounded-full shadow-sm bg-gradient-to-r from-blue-500 to-blue-600"
                     ...
                     <div className="flex items-center space-x-6 text-sm text-gray-500">
                        {comments.length > 0 && (
                           <motion.span
                              className="font-medium transition-colors cursor-pointer hover:text-blue-600"
                              whileHover={{ scale: 1.05 }}>
                  ...
                  <motion.div
                     ...
                     <Button
                        ...
                        <motion.div
                           ...
                           transition={{ duration: 0.3 }}>
                           <ThumbsUp
                              className={`w-5 h-5 ${likes.includes(user?._id) ? "fill-current" : ""}`}
                           />
                        </motion.div>
                        <span>Like</span>
                     </Button>
